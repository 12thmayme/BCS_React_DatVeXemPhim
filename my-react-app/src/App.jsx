...
const App = () => {
  ...
  const handleSeatClick = (seat) => {
    if (seat.gia === 0 || seat.daDat) return; // Don't allow booking already reserved seats

    if (selectedSeats.some((selected) => selected.soGhe === seat.soGhe)) {
                ...
                <div
                  ...
                  className={`seat ${
                    row.hang === ""
                      ...
                      : seat.gia === 0
                      ? ""
                      : selectedSeats.some((s) => s.soGhe === seat.soGhe)
                      ? "selected"
                      : ""
                  }`}
                  onClick={() => handleSeatClick(seat)}
                >
                  {row.hang ? seat.soGhe : seat.soGhe}{" "}
                  {/* Only show seat numbers */}
                </div>
              ))}
            </div>
          ))}
      ...
      <div className="summary">
        ...
        <ul>
          {selectedSeats.map((seat) => (
            <li key={seat.soGhe}>
              {seat.soGhe} - {seat.gia.toLocaleString()} VND
            </li>
          ))}
        </ul>
